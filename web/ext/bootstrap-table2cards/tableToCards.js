/* Important: updated by Mark Worthmann 6.09.2020 / header-labels on filter-columns */
    ...
    $("table.table").each(function (j, table) {
		...
		$(this).find("tbody tr").each(function() {
			...
			$(this).children("td").each(function(i, td) {
                                //edited Mark Worthmann, do not show empty filter columns as header
                                if(td.hasAttribute("data-filtercolumn-empty")) return;
				...
				//if (header.isTitle) { //orginal
				if (header.isTitle && !td.hasAttribute("data-filtercolumn")) {//edited Mark Worthmann
					// Title field
					cardTitle.html(td.innerHTML);
					return;
				}
				//if (header.isSubtitle) {//orginal
				if (header.isSubtitle && !td.hasAttribute("data-filtercolumn")) {//edited Mark Worthmann
					// Subtitle field
					cardSubtitles.push(
